...
It serves as the contract between the Agent Logic and the underlying Python implementation (`src.retrieval`).
...
| **`get_character_events`** | `entity` (str)<br>`event_type` (opt, str) | **Retrieve Major Events**.<br>Use for: "What happened to X?", "How did X achieve Y?". Returns turning points and milestones.<br>Event types: sacrifice, transformation, acquisition, loss, encounter, conflict, revelation, milestone.<br>*Source*: Neo4j Graph (MajorEvent nodes). |
